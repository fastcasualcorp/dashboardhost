...
-- ── registro de accesos (auditoría de seguridad: login / abrir / cerrar caja, IP + dispositivo) ──
-- La IP la captura la Edge Function `acceso` (server-side). Solo la GERENCIA lee (IP = dato personal).
-- Nadie escribe directo (default-deny): inserta la Edge Function con service_role.
